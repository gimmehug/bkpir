...
{
    print_example_banner("BKPIR(L)-NOT");
    ...
    cout << "Computing NOT r1 ..." << endl;
    std::vector<Ciphertext> ct_not_r1(len_mh_vecs);

    for (Ciphertext& result_r_not : ct_not_r1){
        encryptor.encrypt_symmetric(plain_zero_vec,result_r_not); 
    }

    clock_t t_not_start = clock();

    for (size_t i = 0; i < tilde_RB.size(); i++) {

        evaluator.sub_plain(tilde_RB[i],plain_one_vec,ct_not_r1[i]);
        evaluator.negate_inplace(ct_not_r1[i]); 
    }

    clock_t t_not_end = clock();

    int b_logic  = decryptor.invariant_noise_budget(ct_not_r1[0]);  

    for (size_t i = 0; i < tilde_RB.size(); i++) {
        evaluator.transform_to_ntt_inplace(ct_not_r1[i]);
    }

    double t_not = (static_cast<double>(t_not_end) - t_not_start) / CLOCKS_PER_SEC;
    ...
    vector<vector<Ciphertext>> tilde_RBP_NOT(var_l,vector<Ciphertext>(var_f));    
    for (size_t k = 0; k < var_l; k++) {
        for (Ciphertext& rpl : tilde_RBP_NOT[k]){
            encryptor.encrypt_symmetric(plain_zero_vec,rpl);             
        }
    }
    for (size_t k = 0; k < var_l; k++) {
        for (size_t i = 0; i < var_f; i++) {
            evaluator.multiply_plain(ct_not_r1[i],plain_PL[k][i],tilde_RBP_NOT[k][i]); 
            evaluator.relinearize_inplace(tilde_RBP_NOT[k][i], relin_keys);
            evaluator.transform_from_ntt_inplace(tilde_RBP_NOT[k][i]); 
        }
    }
    clock_t t_ip_end = clock();
    
    int b_final  = decryptor.invariant_noise_budget(tilde_RBP_NOT[0][0]); 

    double t_ip =((static_cast<double>(t_ip_end) - t_ip_start)) / CLOCKS_PER_SEC;  
    
    double t_B_NOT = t_ip+t_not+t_select;

    cout << "\n...Finished!" << endl;

    std::cout << "\nPreparation time: " << t_predb << " seconds" << std::endl;
    std::cout << "Selection time: " << t_select << " seconds" << std::endl; 
    std::cout << "NOT operation time: " << t_not << " seconds" << std::endl; 
    std::cout << "Inner product time: " << t_ip << " seconds" << std::endl;
    std::cout << "BKPIR(L)_NOT total server time: " << t_B_NOT << " seconds" << std::endl;
    ...
    data_stream.str("");
    auto size_encrypted_answer_NOT = 0;
    for (size_t k = 0; k < var_l; k++) {
        for (size_t i = 0; i < var_f; i++) {
            while (context.get_context_data(tilde_RBP_NOT[k][i].parms_id())->next_context_data()) {
                evaluator.mod_switch_to_next_inplace(tilde_RBP_NOT[k][i]);
            }
            size_encrypted_answer_NOT += tilde_RBP_NOT[k][i].save(data_stream);
        }
    }
    
    std::cout << "Upload cost: " << size_sym_encrypted_selector/1024 <<" KB" << std::endl; 
    std::cout << "Download cost: "<< size_encrypted_answer_NOT / 1024 << " KB" << std::endl;
