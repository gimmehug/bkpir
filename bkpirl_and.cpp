...
{
    print_example_banner("BKPIR(L)-AND");
    ...
    for (size_t i = 0; i < tilde_us2.size(); ++i) {
        ...
        evaluator.rotate_columns_inplace(tilde_u2_primes[i], galois_keys);
        r_count += 1;
        evaluator.add_inplace(tilde_u2_primes[i],temp2);
        add_count += 1;
    ...
    cout << "Computing r1 AND r2..." << endl;
    std::vector<Ciphertext> ct_r1_and_r2(len_mh_vecs);
    for (size_t i = 0; i < len_mh_vecs; i++){
        encryptor.encrypt_symmetric(plain_zero_vec,ct_r1_and_r2[i]); 
    }
    
    clock_t t_and_start = clock();
    ...
    for (size_t i = 0; i < tilde_RB.size(); i++) {
        evaluator.multiply(tilde_RB[i], tilde_RB2[i], ct_r1_and_r2[i]);
        evaluator.relinearize_inplace(ct_r1_and_r2[i], relin_keys);
    }

    clock_t t_and_end = clock();

    int b_logic  = decryptor.invariant_noise_budget(ct_r1_and_r2[0]);  
    
    for (size_t i = 0; i < tilde_RB.size(); i++) {
        evaluator.transform_to_ntt_inplace(ct_r1_and_r2[i]);
    }
    
    double t_and = (static_cast<double>(t_and_end) - t_and_start) / CLOCKS_PER_SEC;
    ...
    vector<vector<Ciphertext>> tilde_RBP_AND(var_l,vector<Ciphertext>(var_f));    
    for (size_t k = 0; k < var_l; k++) { 
        for (Ciphertext& rpl : tilde_RBP_AND[k]){
    ...
    for (size_t k = 0; k < var_l; k++) {
        for (size_t i = 0; i < var_f; i++) {
            evaluator.multiply_plain(ct_r1_and_r2[i],plain_PL[k][i],tilde_RBP_AND[k][i]);
            evaluator.relinearize_inplace(tilde_RBP_AND[k][i], relin_keys);
            evaluator.transform_from_ntt_inplace(tilde_RBP_AND[k][i]); 
        }
    }
    int b_final  = decryptor.invariant_noise_budget(tilde_RBP_AND[0][0]); 

    clock_t t_ip_end = clock();

    double t_ip =((static_cast<double>(t_ip_end) - t_ip_start)) / CLOCKS_PER_SEC; 
    double t_B_AND = t_ip+t_and+t_select; 
    ...
    std::cout << "AND operation time: " << t_and << " seconds" << std::endl; 
    std::cout << "Inner product time: " << t_ip << " seconds" << std::endl;
    std::cout << "BKPIR(L)_AND total server time: " << t_B_AND << " seconds" << std::endl;
    ...
    data_stream.str("");
    auto size_encrypted_answer_AND = 0;
    for (size_t k = 0; k < var_l; k++) {
        for (size_t i = 0; i < var_f; i++) {
            while (context.get_context_data(tilde_RBP_AND[k][i].parms_id())->next_context_data()) {
                evaluator.mod_switch_to_next_inplace(tilde_RBP_AND[k][i]);
            }
            size_encrypted_answer_AND += tilde_RBP_AND[k][i].save(data_stream);
        }
    }
    
    std::cout << "Upload cost: " << size_sym_encrypted_selector/1024 <<" KB" << std::endl; 
    std::cout << "Download cost: "<< size_encrypted_answer_AND / 1024 << " KB" << std::endl;
